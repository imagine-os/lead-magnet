...
- **Version:** 0.3.0 (pass 2: ops manual, WebMCP surface + CLI, annotation pins, studio intake + A/B, real video-on-scroll, Spanish audit, a11y at the root)

## What is in the box (v0.3.0)
...
| 38 components | `src/components/<tier>/<Name>/` with metas at `/#/dev/components`; every root carries `data-component="<Name>"` so annotation pins name the real component |
| 47 rules | `/#/dev/rules` (core R-C / R-P / R-E + plan R-K, studio R-S, landing R-L, demo R-D, booking R-B, admin R-A, proposal R-R, website R-W) |
| 210 declared actions (136 unique ids = 136 WebMCP tools) | `/#/dev/actions`, the WebMCP surface and the voice vocabulary; `npm run actions -- list | export | run`; handlers read the latest render |
| 18 engine checks | `npm run test:engine` (engine, catalog tiers, the shared slot generator) |
