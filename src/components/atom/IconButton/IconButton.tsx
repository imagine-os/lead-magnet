...
export function IconButton({ icon, label, variant = 'ghost', size = 'md', className = '', type = 'button', ...rest }: IconButtonProps) {
  return <button type={type} className={`iconbtn iconbtn-${variant} iconbtn-${size} ${className}`} aria-label={label} title={label} {...rest}><Icon name={icon} size={size === 'sm' ? 18 : 20} /></button>;
}
