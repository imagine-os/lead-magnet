...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    return (<div className="container page"><div data-component="ErrorBoundary" className="errbox" role="alert"><h1>This page hit an error</h1><pre className="xs" style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{this.state.error.message}</pre><div className="row wrap"><button type="button" className="btn btn-outline btn-sm" onClick={() => this.setState({ error: null })}>Try again</button><a className="btn btn-ghost btn-sm" href="#/">Hub</a></div></div></div>);
  }
}
