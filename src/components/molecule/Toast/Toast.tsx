...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (<Ctx.Provider value={value}>{children}
    <div className="toasts" role="status" aria-live="polite">{items.map((t) => <div key={t.id} className={`toast toast-${t.tone}`}><Icon name={t.tone === 'success' ? 'check' : t.tone === 'danger' ? 'alert' : 'info'} size={18} /><div className="toast-text"><strong>{t.title}</strong>{t.body && <div className="xs">{t.body}</div>}</div><button type="button" className="toast-x" aria-label="Dismiss" onClick={() => dismiss(t.id)}><Icon name="close" size={16} /></button></div>)}</div>
  </Ctx.Provider>);
}
...
export function Toast({ tone = 'info', title, body }: Omit<ToastItem, 'id'>) { return <div className={`toast toast-${tone} toast-static`}><Icon name={tone === 'success' ? 'check' : 'info'} size={18} /><div className="toast-text"><strong>{title}</strong>{body && <div className="xs">{body}</div>}</div></div>; }
