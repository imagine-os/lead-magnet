...
/** A narrower business type within an industry (catalog pass 2, D-063). `key` is what `Prospect.sub_industry` stores; `extra_tools` are catalog tool names (checked against `CATALOG_TOOL_NAMES`) worth guessing in addition to the industry's own `stack`. Additive only - never rename a `key` once it ships, S-02 may already have written it to a prospect. */
export interface SubIndustry { key: string; name: { en: string; es: string }; extra_tools?: string[]; roles?: string[] }

/**
 * D-063 adds `price_reviewed` / `price_source_note` to `StackItem` and `sub` to `Industry`. Both are declared here, not in `../types`, because a catalog worker may only touch `src/engine/catalog/**`; TypeScript's module augmentation lets this file extend the shared interfaces without editing the foundation file. Both fields are optional, so every existing reader (engine, studio) is unaffected.
 */
declare module '../types' {
  interface StackItem {
    /** ISO date this list price was last checked for plausibility. */
    price_reviewed?: string;
    /** Why the price is what it is, when it is not a plain monthly subscription fee. */
    price_source_note?: string;
  }
  interface Industry {
    /** Narrower business types within this industry (e.g. quick service vs full service). Optional: most industries do not need it. */
    sub?: SubIndustry[];
  }
}
