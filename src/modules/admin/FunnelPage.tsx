...
/** A-01 - the whole funnel in one screen: KPIs, the five stages, the same five by archetype / warmth / variant, and what just happened. */
export function FunnelPage() {
  const { t } = useI18n(); const { can } = useSession(); const nav = useNavigate();
  ...
  const variantOf = (e: EventRow) => (e.page_id ? pageById[e.page_id]?.variant : undefined);

  const shown = useMemo(() => events.filter((e) => (arch === 'all' || archOf(e) === arch) && (warm === 'all' || warmOf(e) === warm)), [events, arch, warm, pageById, prospectById]);
  ...
  const variantGroups = groupSessions(shown, variantOf);
  const liveVariants = new Set(pages.filter((g) => g.status === 'live').map((g) => g.variant));
  const abReady = liveVariants.size >= 2 && variantGroups.length >= 2; // R-A02
  const recent = useMemo(() => [...events].sort((a, b) => String(b.ts).localeCompare(String(a.ts))).slice(0, 12), [events]);

  const api = useRef({ setArch, setWarm });
  api.current = { setArch, setWarm };
  useActions('A-01', {
    'admin.filterFunnel': (p) => {
      const a = String(p?.archetype ?? ''); const w = String(p?.warmth ?? '');
      ...
      if (!a && !w) { api.current.setArch('all'); api.current.setWarm('all'); }
      return { archetype: a || 'all', warmth: w || 'all' };
    },
  ...
  return (<div className="container container-wide page stack ad">
    ...
    <Card className="stack-sm"><div className="row wrap"><h2>{t('admin.ab_h2')}</h2><Badge size="sm" tone={abReady ? 'success' : 'warn'}>{abReady ? t('admin.ab_ready') : t('admin.ab_blocked')}</Badge></div>
      {abReady
        ? <DataTable caption={t('admin.ab_h2')} dense rows={toBreakRows(variantGroups, (k) => t('admin.variant', { v: k }))} columns={breakCols} />
        : <EmptyState icon="target" title={t('admin.ab_need_title')} body={t('admin.ab_need_body', { n: String(liveVariants.size), variants: [...liveVariants].sort().join(', ') || '—' })} action={can('pages.publish') ? <Link to="/studio"><Button variant="outline" size="sm" icon="sparkles">{t('admin.ab_go_studio')}</Button></Link> : undefined} />}
