...
5. **proof** - four facts about this prospect, three claims they can check in the next two minutes, and an honestly-labelled sample testimonial (the invented logo strip was removed in pass 3).
...
- **Every role view is forwardable.** Each card carries "Send the &lt;role&gt; view" beside "Try it as &lt;role&gt;". It
  copies the absolute deep link to that view (`<base>#/demo/:prospectId/role/:slug`) with the async clipboard API and
  then shows the link in a focused, selected, read-only field **either way**: on success the field confirms what was
  copied, on a refused clipboard it is the fallback ("press Ctrl / Cmd + C"). Tracked as
  `cta_click { cta: 'share_role', role, role_kind, copied }`. The prospect is rarely the only decision-maker, and the
  manager's own screen is a better forward than a page about software (R-L07).
- **Honest expiry beside every CTA.** `<ExpiryLine>` prints the real `expires_at` under the hero CTAs, under the
  booking grid and (as the badge line) in the CTA band: "Live until September 30 - 11 days left. No countdown, no
  pressure: that is just when we take it down." The phone sticky bar appends "live 11 more days" to the savings
  number. There is no timer anywhere on the page (R-L08).
- **The savings number is correctable from here.** "These numbers wrong? Fix them" moves to `/p/:slug/audit`
  (`landing.correctStack`), because a number they can correct beats a number they must believe.
- **Proof without invented logos.** The five placeholder logo marks are gone; in their place, "Three things you can
  check in the next two minutes" - the demo is the running product, the guessed stack is theirs to cross out, and the
  page really comes down on that date. The sample quote stays, still labelled "Sample, not a customer", and
  `landing.seeCaseStudy` stays a Placeholder.
- **Four switching objections in the FAQ**, appended to the composed questions: annual contracts, the data in a
  dropped tool (automatic import is **not built**, and the answer says so), something breaking mid-switch, and a team
  that will not learn another system. They live in the module strings table until `composePage()` can carry them.
- **Exit intent on touch** also fires on a fast flick back up the page (past a quarter of it, two samples above
  1400 px/s), alongside the back gesture and the 45 s idle timer. Once per session, still the calendar, never a
  discount.
- **Driveable without a pointer.** `useSpatialNav(rootRef)` + `useGamepadNav(spatial)` on the page root: arrow keys, a
  TV remote d-pad and a gamepad reach every control, and Back returns to the top of the page (R-L09).
...
- `docs/changelog/0023-integration-pass-3.md` (pass 3 integration: the four switching objections are composed by `composePage()` (D-109); the role share button uses the `link` icon; the RoleViews strip is `data-spatial="skip"`; savings `Stat` sizes through `--stat-value-size`)
- `docs/changelog/0024-reference-items.md` + `0025-os-demo-depth.md` + `0026-integration-reference-items.md` (reference items, v0.5.0: `HeroReveal` and `OgCard` read `industryFor(prospect)` so the hero fact and the OG line follow the sub-industry; `MiniOs` renders `meter` widgets with the library `Meter` (`size="sm"`, `tone="prospect"`, host-sized through `--meter-*`, D-138); three new seeded slugs `fetch-and-stay-san-diego`, `renters-shield-law-fresno`, `raiz-wellness-san-antonio` with frames and OG cards; `SEED_VERSION` 5)
