...
export function ProgressBar({ value, max = 100, label, showValue = true, tone = 'primary', size = 'md' }: ProgressBarProps) {
  ...
  return (<div data-component="ProgressBar" className={`progress progress-${size}`}><div className="progress-head xs"><span>{label}</span>{showValue && <span className="muted">{Math.round(pct)}%</span>}</div>
