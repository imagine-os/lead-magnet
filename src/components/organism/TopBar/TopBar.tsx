...
/** Staff top bar: menu (narrow), title, Commands (the palette, also Ctrl/Cmd+K), language, theme, user. */
export function TopBar({ title, onMenu }: TopBarProps) {
  const { user, role, viewAs, devMode } = useSession(); const { theme, toggleTheme } = useTheme(); const t = useT();
  return (<header data-component="TopBar" className="topbar">
    ...
    <IconButton icon="search" label={`${t('control.open')} · ${t('control.shortcut')}`} onClick={() => openPalette()} className="topbar-commands" />
    <LangToggle size="sm" />
