...
export function Chip({ children, selected, onClick, onRemove, removeLabel = 'Remove', className = '' }: ChipProps) {
  ...
  if (onClick) return <button type="button" className={`chip ${selected ? 'is-selected' : ''} ${className}`} aria-pressed={selected} onClick={onClick}>{inner}</button>;
  return <span className={`chip ${selected ? 'is-selected' : ''} ${className}`}>{inner}</span>;
}
