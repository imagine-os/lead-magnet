 ...
 * the request arrives inside a user gesture (see useVoice.start); otherwise it focuses the microphone button.
 */
import { useSyncExternalStore } from 'react';

export interface PaletteState { open: boolean; query: string; /** Start listening (gesture permitting) as soon as the palette is open. */ voice: boolean; /** Increments per open() so an already-open palette still reacts. */ seq: number }
let state: PaletteState = { open: false, query: '', voice: false, seq: 0 };
const listeners = new Set<() => void>();
const emit = () => listeners.forEach((l) => l());
const subscribe = (l: () => void) => { listeners.add(l); return () => { listeners.delete(l); }; };
const get = () => state;

export function openPalette(req: { query?: string; voice?: boolean } = {}): void { state = { open: true, query: req.query ?? '', voice: !!req.voice, seq: state.seq + 1 }; emit(); }
export function closePalette(): void { if (!state.open) return; state = { ...state, open: false, voice: false }; emit(); }
