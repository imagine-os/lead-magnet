...
import { FIELD_WEIGHTS, computeConfidence, industry, subIndustries, type Bi } from '../../engine';
...
export const SELECT_FIELDS = ['industry', 'sub_industry', 'warmth', 'lang', 'revenue_band'];
...
export function fieldOptions(field: string, t: (k: string, v?: Record<string, string | number>) => string, bi: (v: Bi | string) => string, p?: ProspectRow): { value: string; label: string }[] {
  if (field === 'industry') return industryOptions(bi);
  if (field === 'sub_industry') return p ? subIndustries(p).map((s) => ({ value: s.key, label: bi(s.name) })) : [];
...
export const confidenceGain = (p: ProspectRow, field: string): number => Math.round((computeConfidence([...(p.fields_known ?? []), field], p) - p.confidence) * 100);
