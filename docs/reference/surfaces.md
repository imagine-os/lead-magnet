...
51 routes, 51 built, 0 stubs (v0.4.0; unchanged since 0.3.0). Every shell mounts `useSpatialNav` + `useGamepadNav` except L-06 (hidden static card); `CommandPalette` (Ctrl/Cmd+K) is mounted once by `ControlBridge` on every route. `public/frames/<prospectId>/` (video-on-scroll JPEG sequences from `npm run frames`) and `public/og/*.jpg` are the static assets the public pages read; together <= 5 MB.

### 1.2 Data: DataProvider
`src/data/provider.ts`; `MockProvider` today (`SEED_VERSION` 4: 0.4.0 reseeds stale browsers - the seeded A/B splits, the recomposed pages with the objections `faq`, the demo / landing copy - rows a person created are carried over). Tables (11): prospects, stack_guesses, pages, events, bookings (+ `contact_name`, `contact_email`, `contact_phone` since 0.2.0), assets, touches, tasks, feedback, recommendations (`kind` gained `promote_variant` in 0.4.0: A-02 writes the four engine kinds, A-01 writes `promote_variant` when an A/B side is ahead past the sample, insert only, status `proposed`), **`intake_turns`** (since 0.3.0: every intake answer with its source - S-02 `IntakeChat` writes it, S-02 / A-01 read it). `supabase/schema.sql` + `docs/data-model.md` regenerated in 0.4.0 (`intake_turns` had been missing from the SQL). Tables written from the product now: `tasks` (K-01 / K-04 `status`, `done_at`), `pages` (S-03, A-02), `prospects` / `stack_guesses` (S-02, L-02), `bookings` (B-01 insert, A-05 status), `touches` (S-05, A-04), `assets` (S-04), `recommendations` (A-02), `events` (every page via `track()`).
...
Engine (pure, `src/engine`, `npm run test:engine` 21 checks - D-063 catalog coverage and the sub-industry intake since 0.4.0): `guessStack, savings, deriveRoleViews, pickArchetype, composePage, imagePrompts, nextQuestions, applyAnswer, adaptFromEvents`, since 0.3.0 `stackTier` (likely / possible, D-056), `itemCost`, `paidLocations`, `CATALOG_TOOLS`, `CATALOG_TOOL_NAMES`, `INDUSTRY_KEYS` (`IndustryKey` is `keyof` the catalog, so adding an industry is one property in `catalog/industries.ts`), since 0.4.0 the catalog's `Industry.sub` (`SubIndustry { key, name, extra_tools?, roles? }`, 24 across 9 industries, `docs/reference/catalog.md`), `StackItem.price_reviewed` / `price_source_note`, `subIndustries(p)`, `computeConfidence(fields_known, p?)` (a field that does not apply to the prospect leaves the denominator, D-122), and the voice matcher `matchPhrase` / `fillPhrase` (`src/a11y/voice.ts`, `npm run test:voice` 18 checks), tuning constants `TIER_MIN_LIKELY / RARE_PREVALENCE / POSSIBLE_MIN_TEAM` plus **`slots`** since 0.2.0: `tzForProspect(p)`, `slotGrid(prospectId, tz, from?, days?)`, `previewSlots(grid, perDay?)`, `withRequested(grid, iso)`, `findSlot`, `slotInWords`, `dayLabel`, `timeLabel`, `zonedInstant`, `DURATION_MIN`. The landing `booking_inline` section, B-01 / B-02 and A-05 all import from here (D-051); T42 replaces `slotGrid` and the `bookings` insert only.
...
| `test:engine` | pure engine checks (esbuild bundle), 21 | |
| `test:voice` | pure `matchPhrase` checks over `docs/reference/voice-vocabulary.json` (esbuild bundle, no browser), 18 | |
...
| `qa:dpad` | arrow-keys-only walk of every built route at 1920 -> `docs/qa/dpad-report.{md,json}` (reachable / candidates, unreached primary CTAs, dead ends, arrow owners, Backspace, pages without the hook); advisory until T58 makes `--strict` a gate | `--only=`, `--codes=`, `--width=`, `--port=`, `--server=dev\|preview\|auto`, `--max-steps=`, `--strict` |
...
Every route's `spec.actions` (`id, label, intent, permission?, params?`), published in `window.__leadmagnet.routes[].spec.actions`, browsable and runnable at `/#/dev/actions` (D-04: WebMCP status, per-action Run with a params form from the JSON Schema, voice phrase). Handlers are registered while a page is mounted with `useActions(code, map)` and always read the latest render (D-053). 230 declared rows across 51 routes, 144 unique ids on 2026-09-19 v0.4.0 (`npm run actions -- list`; 227 at the integration commit, +3 when the pass-3 review declared `landing.toggleFaq` on L-02..L-04; 210 / 51 / 136 at v0.3.0, 175 / 50 / 117 at T20); an id shared by several pages, e.g. `booking.openDemo`, is one tool with several hosts. Static export: `docs/reference/actions-manifest.json`, `docs/reference/voice-vocabulary.json` (`npm run actions -- export`). Consumers: `navigator.modelContext` (WebMCP), the CLI, and since 0.4.0 the `CommandPalette` (typed or spoken phrase -> `matchPhrase` -> `runAction`).

| Prefix | Pages | Unique ids | Ids |
| --- | --- | --- | --- |
| `admin.*` | A-01, A-02, A-03, A-04, A-05 | 15 | `applyRecommendation`, `clearEventFilters`, `compareVariants`, `dismissRecommendation`, `filterEvents` (+ `variant`), `filterFunnel` (+ `range`), `filterOutreach`, `moveTouch`, `openEventMeta`, `openFor`, `openProspect`, `openStudio`, `recordAbWinner`, `recordRecommendation`, `setBookingStatus` |
| `booking.*` | B-01, B-02 | 7 | `addToCalendar`, `clearSlot`, `confirm`, `openDemo`, `openPage`, `pickSlot`, `setLang` |
| `demo.*` | C-01, C-02, C-03, C-04, C-05, C-06, C-07 | 15 | `bookCall`, `filterChannel`, `goto`, `invite`, `openDepartment`, `openLifeRole`, `openPayment`, `openRole`, `openThread`, `openWidget`, `quickAction`, `reply`, `saveWorkspace`, `shareView`, `switchRole` |
...
| `hub.*` | HUB-01, HUB-02 | 9 | `openCommands`, `openSurface`, `resetDemoData`, `setLang`, `signInAs`, `switchUser`, `toggleDevMode`, `toggleTheme`, `voiceListen` |
| `landing.*` | L-01, L-02, L-03, L-04, L-05, L-06 | 18 | `bookCall`, `confirmTool`, `correctStack`, `goToStep`, `openDemo`, `openPage`, `pickSlot`, `playLetter`, `rejectTool`, `requestRefresh`, `resetTool`, `saveWorkspace`, `seeCaseStudy`, `setLang`, `shareRole`, `toggleFaq`, `tryAsRole`, `viewRole` |
...
### 2.2a Voice controller (T46) - shipped in 0.4.0, see `control.md` §4
`matchPhrase` (`src/a11y/voice.ts`, pure, `npm run test:voice`) + `useVoice` (Web Speech, starts only inside a user gesture, D-100) + `CommandPalette` (the voice screen, D-101: what is heard is ranked, gated and run like a typed phrase; `hub.openCommands` / `hub.voiceListen`). Remaining: ES-native intents (T57), hold-to-talk on a TV remote's mic once a remote adapter exists, per-page phrase hints, a `dev.confirm` action so a spoken "confirm" answers a `ConfirmDialog`. Remote / gamepad d-pad (T47) - shipped: `useSpatialNav` + `useGamepadNav` on every shell (hub, demo, `DesktopShell` with per-axis arrow ownership and Escape parking, landing L-01..L-05, booking, website, proposal, no-access; D-102, D-124); `npm run qa:dpad` rehearses it (`docs/qa/dpad-report.md`); `--strict` becomes a gate in T58.
...
- 2026-09-19 · prompt 0004 (pass-3 sweep + final pass, changelog 0023 "Sweeps") · scripts: `qa:responsive`, `qa:a11y`, `screenshots`, `frames` boot on `#root > *:not(dialog)` (D-125; the palette's closed dialog was matched first and every load hit the 10 s timeout); `qa:a11y` settles animations and same-origin iframes before axe / the walk and reads hit targets to 0.1 px (rules in the report header); `test:voice` 20 checks (+ `routeIdFor`, apostrophe-safe `quoted`); `hub.voiceListen` from an agent / CLI opens the palette with the Speak button focused and never the microphone (D-126); `landing.toggleFaq` declared on L-02..L-04 (230 declared rows, 144 ids); `matchPhrase` is asked for 64 rows and the palette cuts to 12 after `can()`.
- 2026-09-19 · prompt 0004 (pass 3, v0.4.0, changelogs 0017..0023) · §1.1 spatial hook on every shell + the global `CommandPalette`; §1.2 `recommendations.kind` `promote_variant` (A-01 second writer), `SEED_VERSION` 4, SQL regenerated, engine 21 checks + `Industry.sub` / `subIndustries` / `computeConfidence(fields_known, p?)` / `matchPhrase`; §1.3 `test:voice`, `qa:dpad`, `catalog:doc`; §1.4 regenerated from `npm run actions -- list --json`: 227 rows / 51 routes / 144 unique ids (admin +2 `compareVariants` `recordAbWinner`, demo +1 `shareView`, hub +2 `openCommands` `voiceListen`, landing +3 `shareRole` `correctStack` `resetTool`); §1.6 144 tools, the palette as third consumer; §2.2a voice + d-pad shipped.
