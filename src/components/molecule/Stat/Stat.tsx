...
export function Stat({ label, value, hint, tone = 'default', size = 'md' }: StatProps) {
  return <div data-component="Stat" className={`stat stat-${tone} stat-${size}`}><div className="stat-label eyebrow">{label}</div><div className="stat-value font-display">{value}</div>{hint && <div className="stat-hint xs muted">{hint}</div>}</div>;
}
