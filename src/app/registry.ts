...
/**
 * Collects every module: src/modules/<name>/index.ts exporting { routes, strings }. Nobody edits this file to add a page.
 * When two modules register the same path, a built page wins over a PageStub (so `_stubs` pre-registers every planned code).
 * Getters are lazy on purpose (ESM cycle safety): call getRoutes() inside functions / components, never at module top level.
 */
...
function build() {
  ...
  const modules = Object.entries(found).map(([path, m]) => ({ name: path.split('/')[2], routes: m.routes ?? [], strings: m.strings ?? {} })).sort((a, b) => a.name.localeCompare(b.name));
...
export const routeStatus = (r: RouteDef): 'built' | 'stub' => (isStubElement(r.element) ? 'stub' : 'built');
