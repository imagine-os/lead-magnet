 ...
 * container or nothing is focused; never inside inputs, selects, textareas, sliders, combos, iframes or `[data-spatial="skip"]`
 * (the K-03 graph owns its arrows), and never while a modal dialog is open. Native Tab order is untouched.
 */
export function useSpatialNav(ref: RefObject<HTMLElement | null>, opts: SpatialNavOptions = {}): SpatialControls {
  ...
  const eligible = useCallback((): { root: HTMLElement; active: Element | null; inside: boolean } | null => {
  ...
  const move = useCallback((dir: Direction): boolean => {
    ...
    const target = e.inside && e.active ? nearest(e.active.getBoundingClientRect(), cands, dir) : firstByPosition(cands);
