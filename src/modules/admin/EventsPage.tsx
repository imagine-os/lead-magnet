...
import { normalizeVariant, variantOfEvent, VARIANTS } from './ab';
import { EVENT_ICON, eventSummary, when } from './timeline';
import './admin.css';

const ALL = '';
/** A-03 - the raw events log with the five filters that matter (type, page, A/B variant, session, prospect) and the full meta JSON per row. */
export function EventsPage() {
  ...
  const [type, setType] = useState(ALL); const [pageId, setPageId] = useState(ALL); const [session, setSession] = useState(ALL); const [prospectId, setProspectId] = useState(ALL); const [variant, setVariant] = useState(ALL);
  ...
  /** A/B: the event's variant is its `pages` row, falling back to the `variant` the landing page stamped in meta. */
  const varOf = (e: EventRow) => variantOfEvent(e, pageById);
  const rows = useMemo(() => events
    .filter((e) => (!type || e.type === type) && (!pageId || e.page_id === pageId) && (!session || e.session_id === session) && (!prospectId || e.prospect_id === prospectId) && (!variant || varOf(e) === variant))
    .sort((a, b) => String(b.ts).localeCompare(String(a.ts))), [events, type, pageId, session, prospectId, variant, pageById]);

  const api = useRef({ setType, setPageId, setSession, setProspectId, setVariant, setOpen, rows });
  api.current = { setType, setPageId, setSession, setProspectId, setVariant, setOpen, rows };
  useActions('A-03', {
    'admin.filterEvents': (p) => {
      ...
      if (p?.variant !== undefined) { const v = String(p.variant ?? ''); api.current.setVariant(v ? normalizeVariant(v) : ''); }
      return { type, page: pageId, session, prospect: prospectId, variant };
    },
    'admin.clearEventFilters': () => { api.current.setType(''); api.current.setPageId(''); api.current.setSession(''); api.current.setProspectId(''); api.current.setVariant(''); return 'cleared'; },
  ...
  const columns: Column<EventRow>[] = [
    { key: 'ts', header: t('admin.ev_when'), width: '12rem', sortable: true, accessor: (r) => String(r.ts), render: (r) => <span className="xs">{when(String(r.ts))}</span> },
    { key: 'type', header: t('admin.ev_type'), sortable: true, accessor: (r) => t(`admin.ev_${r.type}`), render: (r) => <span className="row xs"><Icon name={EVENT_ICON[r.type] ?? 'info'} size={14} />{t(`admin.ev_${r.type}`)}</span> },
    { key: 'prospect', header: t('admin.ev_prospect'), sortable: true, accessor: (r) => (r.prospect_id && prospectById[r.prospect_id] ? (prospectById[r.prospect_id] as ProspectRow).business_name : null), render: (r) => (r.prospect_id && prospectById[r.prospect_id] ? <Link to={`/admin/prospects/${r.prospect_id}`}>{(prospectById[r.prospect_id] as ProspectRow).business_name}</Link> : <span className="xs muted">—</span>) },
    { key: 'page', header: t('admin.ev_page'), sortable: true, accessor: (r) => (r.page_id && pageById[r.page_id] ? (pageById[r.page_id] as PageRow).slug : null), render: (r) => <span className="xs">{r.page_id && pageById[r.page_id] ? (pageById[r.page_id] as PageRow).slug : '—'}</span> },
    { key: 'variant', header: t('admin.ev_variant'), sortable: true, accessor: (r) => varOf(r) ?? null, render: (r) => { const v = varOf(r); return v ? <Badge size="sm">{t('admin.variant', { v })}</Badge> : <span className="xs muted">{t('admin.ev_no_variant')}</span>; } },
    ...
    { key: 'open', header: t('admin.ev_json'), align: 'right', hideOnCard: true, srOnlyHeader: true, render: (r) => <Button size="sm" variant="ghost" icon="code" onClick={() => setOpen(r)}>JSON</Button> },
  ];
  ...
  return (<div className="container container-wide page stack ad">
    ...
    <Card padding="sm">
      <div className="ad-filter-grid">
        ...
        <Field label={t('admin.ev_variant')} hint={t('admin.ev_variant_hint')}><Select value={variant} onChange={(e) => setVariant(e.target.value)} options={[{ value: ALL, label: t('admin.all') }, ...VARIANTS.map((v) => ({ value: v, label: t('admin.variant', { v }) }))]} /></Field>
    ...
    <DataTable caption={t('admin.events_h1')} dense rows={rows} columns={columns} defaultSort={{ key: 'ts', dir: 'desc' }} onRowClick={(r) => setOpen(r)} empty={{ title: t('admin.no_events'), body: t('admin.no_events_body') }} />
    <Drawer open={!!open} onClose={() => setOpen(null)} title={open ? t(`admin.ev_${open.type}`) : t('admin.ev_meta')} width={520}>
      {open && (<div className="stack-sm">
        ...
        <dl className="ad-dl">
          ...
          <dt>{t('admin.ev_variant')}</dt><dd>{varOf(open) ? t('admin.variant', { v: varOf(open) as string }) : t('admin.ev_no_variant')}</dd>
