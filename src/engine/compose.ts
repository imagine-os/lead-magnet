...
export function composePage(p: Prospect, archetype: Archetype, opts: ComposeOpts = {}): PageModel {
  ...
  const order: Record<Archetype, Section[]> = {
    ...
    audit: [{ ...hero, headline: bi(`${B} is paying about ${usd(sav.monthly_current)} a month for tools that do not talk to each other.`, `${B} paga cerca de ${usd(sav.monthly_current)} al mes por herramientas que no se hablan.`) }, audit, savingsSec, rolesSec, proof, band, booking],
    walkthrough: [{ ...hero, headline: bi(`This is Tuesday at ${B}, next week.`, `Así es un martes en ${B}, la próxima semana.`) }, steps, rolesSec, savingsSec, { ...band, headline: bi('Make it next Tuesday', 'Que sea el próximo martes') }, booking],
    letter: [letter, hero, band, booking],
  };
