...
// Pass 3. The prospect is rarely the only decision-maker: every role view has a link that opens straight into it.
const shareRole: ActionDef = { id: 'landing.shareRole', label: 'Send this role view', intent: 'copy the link to the {role} view', params: { role: 'string' }, permission: 'demo.open' };
const correctStack: ActionDef = { id: 'landing.correctStack', label: 'Correct the stack', intent: 'let me correct the tools you guessed', permission: 'demo.open' };
const resetTool: ActionDef = { id: 'landing.resetTool', label: 'Undo this answer', intent: 'undo my answer about {tool}', params: { tool: 'string' } };
...
const COMMON = {
  ...
  states: ['loading', 'live', 'expired -> L-05', 'reduced motion', 'phone sticky CTA', 'exit intent shown', 'variant A', 'variant B', 'frames present', 'frames missing (MiniOs fallback)', 'role link copied', 'clipboard refused (link selected)', 'no expires_at'],
  checkedAt: WIDTHS,
...
const MECHANICS = [
  ...
  'honest expiry (R-C04) is one component, <ExpiryLine>, rendered under the hero CTAs, in the CTA band, under the booking grid, in the letter and in the sticky bar: the real expires_at as a date plus days left, never a countdown; a row with no expiry says so',
  'exit intent, once per session: fine pointer = the pointer leaves through the top; coarse pointer = the back gesture, a fast flick back up the page past a quarter of it, or 45 s idle. Nothing hover-only (P-03)',
  'useSpatialNav(rootRef) on the page root: arrow keys / d-pad move focus geometrically, Enter activates, Back returns to the top of the page (a public page has no level above it). Cards keep their own Left / Right, inputs keep their arrows, an open dialog turns it off',
];
...
export const revealSpec: PageSpec = defineSpec({
  ...
  layout: ['top bar (wordmark, EN/ES, primary CTA)', 'hero_reveal (3 devices, scroll-driven)', 'savings_stack (struck-through prices, count-up)', 'role_views (snap carousel / grid, per-role share link)', 'proof (facts + three checkable claims + labelled sample)', 'faq (composed questions + four switching objections)', 'cta_band (honest urgency)', 'booking_inline (7-day grid)', 'sticky CTA (phone)', 'exit intent modal', 'save workspace modal'],
  logic: [...MECHANICS, 'hero devices scrub a real frame sequence captured from the live demo: useFrames(prospectId) loads public/frames/<id>/index.json and preloads it, useScrollFrames(count) maps the hero section to a frame (phone = NN.jpg, laptop = desk-NN.jpg), and when the index is missing the devices render the live <MiniOs> composition instead', 'reduced motion pins the first frame and never animates; the TV always holds the live composition', 'every role card copies /#/demo/:prospectId/role/:slug with the async clipboard API and shows the link in a selected read-only field either way (cta_click { cta: share_role, role })', 'the FAQ appends four switching objections from the module strings table on top of the composed questions'],
  components: ['DeviceMockup', 'Stat', 'Button', 'IconButton', 'Card', 'Badge', 'Modal', 'Field', 'Input', 'LangToggle', 'Placeholder', 'Toast', 'Avatar'],
  actions: [openDemo, bookCall, saveWorkspace, setLang, viewRole, tryAsRole, shareRole, correctStack, toggleFaq, seeCase, pickSlot],
  rules: ['R-C01', 'R-C02', 'R-C03', 'R-C04', 'R-C05', 'R-C06', 'R-L01', 'R-L02', 'R-L03', 'R-L04', 'R-L05', 'R-L07', 'R-L08', 'R-L09'],
});
...
export const expiredSpec: PageSpec = defineSpec({
  ...
  rules: ['R-C05', 'R-L05', 'R-L08'],
  states: ['expired page', 'unknown slug', 'request sent', 'known prospect (booking offered)', 'unknown slug (booking is a Placeholder)'],
});
