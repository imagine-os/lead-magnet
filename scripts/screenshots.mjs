...
// Usage: npm run build && npm run screenshots [-- --codes=HUB-01,D-07] [-- --only=/dev] [-- --widths=390,1280] [-- --dark] [-- --label=before] [-- --smoke] [-- --port=4173]
...
async function main() {
  ...
  for (const { path, code } of list_) {
    ...
    for (const width of WIDTHS) for (const theme of !SMOKE && (ALL_DARK || KEY_PAGES.has(code)) ? ['light', 'dark'] : ['light']) {
      ...
      try { await page.goto(`${BASE}${url}`, { waitUntil: 'load', timeout: 20000 }); await page.waitForSelector('#root > *', { timeout: 10000 }); await page.waitForTimeout(500);
